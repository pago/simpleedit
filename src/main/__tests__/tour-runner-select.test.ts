...
// tour.ts → runner.ts → models/ollama.ts imports `net` from electron; tour.ts
// itself imports `app` from electron for the cache dir.
vi.mock('electron', () => ({
  net: { fetch: vi.fn() },
...
import { selectRunner } from '../tour'
import { ClaudeCodeRunner, DirectRunner } from '../agent-tasks/runner'

const getModelConfigMock = vi.mocked(getModelConfig)

function config(tour?: ModelConfig['defaults']['tour']): ModelConfig {
  return { defaults: tour ? { tour } : {}, submenuAllowlist: [] }
}
